...
class CosineEstimator(torch.autograd.Function):
    ...
    def forward(ctx, grid_indices_here, posterior):
      ...
      #   print(f"CosineEstimator:{posterior}")
      #   print(f"CosineEstimator:{torch.isnan(posterior).any()}")
        ...
        for itera in range(30):
         ...
         #  print(f"Mask0:{MASK}")
         #  print(f"updateGD0:{updateGD}")
         #  print(f"updateNewton0:{updateNewton}")
...
class CosineEstimator1(torch.autograd.Function):
    """
    We can implement our own custom autograd Functions by subclassing
    torch.autograd.Function and implementing the forward and backward passes
    which operate on Tensors.
    ...
    def forward(ctx, grid_indices_here, posterior):
        ...
        """
        grid_indices_here = grid_indices_here * 2 * math.pi/GRID
      #   print(f"CosineEstimator1:{posterior}")
      #   print(f"CosineEstimator1:{torch.isnan(posterior).any()}")
        n_inputs, n_batch = posterior.size()
        initialized = (grid_indices_here.data * posterior).detach().sum(dim=0).data.clone()
        result = initialized.clone()
        ...
        for itera in range(30):
          ...
          if OPTIMIZER_VERBOSE:
            loss = (((1-SQUARED_SENSORY_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P1/2)) * posterior.detach()).sum(dim=0)
          loss_gradient = (P1/2 * (SQUARED_SENSORY_DIFFERENCE(result.unsqueeze(0) - grid_indices_here)) * ((1-SQUARED_SENSORY_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P1/2-1)) * posterior.detach()).sum(dim=0)

          loss_gradient2 = (P1/2 * (SQUARED_SENSORY_SIMILARITY(result.unsqueeze(0) - grid_indices_here)) * ((1-SQUARED_SENSORY_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P1/2-1)) * posterior.detach()).sum(dim=0) / 1
          if P1 == 2:
             pass
          else:
             loss_gradient2 = loss_gradient2 + (P1/2 * (P1/2-1) * (SQUARED_SENSORY_DIFFERENCE(result.unsqueeze(0) - grid_indices_here)).pow(2) * ((1-SQUARED_SENSORY_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P1/2-2)) * posterior.detach()).sum(dim=0) / 1

          MASK = (loss_gradient2 > 0.001)
          updateGD = - loss_gradient /loss_gradient2.abs().clamp(min=0.0001)
          updateNewton = - loss_gradient/loss_gradient2
         #  print(f"Mask1:{MASK}")
         #  print(f"updateGD1:{updateGD}")
         #  print(f"updateNewton1:{updateNewton}")
    ...
    def backward(ctx, grad_output):
        ...
        F = (P1/2 * (SQUARED_SENSORY_DIFFERENCE(result.unsqueeze(0) - grid_indices_here)) * ((1-SQUARED_SENSORY_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P1/2-1)) * posterior.detach()).sum(dim=0)

        assert F.abs().mean() < 0.01, (F, F.abs().mean(), F.abs().max()/n_batch, n_batch, ((SQUARED_SENSORY_DIFFERENCE(result.unsqueeze(0) - grid_indices_here)) * posterior.detach()).sum(dim=0) / n_batch)

        dF_posterior = P1/2 * (SQUARED_SENSORY_DIFFERENCE(result.unsqueeze(0) - grid_indices_here)) * ((1-SQUARED_SENSORY_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P1/2-1))
        dF_result = 0
        dF_result = dF_result + (P1/2 * (SQUARED_SENSORY_SIMILARITY(result.unsqueeze(0) - grid_indices_here)) * ((1-SQUARED_SENSORY_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P1/2-1)) * posterior.detach()).sum(dim=0)
        if P1 == 2:
           pass
        else:
           dF_result = dF_result + (P1/2 * (P1/2-1) * (SQUARED_SENSORY_DIFFERENCE(result.unsqueeze(0) - grid_indices_here)).pow(2) * ((1-SQUARED_SENSORY_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P1/2-2)) * posterior.detach()).sum(dim=0)
        if OPTIMIZER_VERBOSE:
           if torch.isnan(result).any():
                print(result.size(), dF_result.size())
                assert False, result
           ...
           if torch.isnan(dF_result).any():
                print(result[torch.isnan(dF_result)])
                print((SQUARED_STIMULUS_DIFFERENCE(result.unsqueeze(0) - grid_indices_here)).pow(2))
                print(((1+1e-10-SQUARED_STIMULUS_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P1/2-2)))
                print(result.size(), dF_result.size(), posterior.size())
                assert False
