...
from loadReisenegger_P1 import *
...
COs = MakeFloatTensor([-35, 0, 35]) #torch.unique(centralOrientationPerTrial)
CO_size = COs.size()
N_CO = CO_size[0]
...
#   regularizer4 = prior_logits_effect_weight * (priorByCOFourierTimesWeightInclMean).pow(2).sum() - 0.5 * (N_CO*2*FOURIER_BASIS_SIZE) * log_prior_logits_effect_weight
   ...
   for CONDITION in [1]:
    for CO in range(N_CO):
     if (centralOrientationPerTrial == COs[CO]).sum() == 0:
         continue
     ...
     loss_model, bayesianEstimate_model, bayesianEstimate_sd_byStimulus_model, attraction, encodingBias = computeBias(xValues, init_parameters["sigma_logit"][CO,CONDITION], prior, volume, n_samples=1000, grid=grid, responses_=observations_y, parameters=parameters, computePredictions=(iteration%PLOT_EVERY == 0), condition_=CONDITION, folds=trainFolds, lossReduce='sum', centralOrientation=CO)
     loss += loss_model
...
#       axis[CO,4].scatter(grid_centered.cpu(), (attraction).detach().cpu())
       ...
       _, bayesianEstimate_2_4_repulsion, _, _, _ = computeBias(xValues, init_parameters["sigma_logit"][CO,CONDITION], 1/GRID+MakeZeros(GRID), volume, n_samples=1000, grid=grid, responses_=observations_y, parameters=parameters, computePredictions=(iteration%PLOT_EVERY == 0), sigma_stimulus=0, sigma2_stimulus=0, condition_=CONDITION, folds=trainFolds, lossReduce='sum', centralOrientation=CO)
