...
#  print(mapping_likelihoods)
  ...
  if lossReduce == 'mean':
    ...
    if condition_ == 0:
      #assert motor_likelihoods.log().abs().max() < 1e20, motor_likelihoods.log().abs().max()
      loss = -torch.gather(input=overall_likelihood,dim=1,index=stimulus.unsqueeze(1)).squeeze(1).log().sum()
      print(stimulus.size(), responses.size(), "SIZES")
#      quit()
      print((-torch.gather(input=overall_likelihood,dim=1,index=stimulus.unsqueeze(1)).squeeze(1) == 0).nonzero())
#      assert loss.abs() < 1e10, (loss, (stimulus-responses)[710:720])
    else:
      assert overall_likelihood.log().abs().max() < 1e20, motor_likelihoods.log().abs().max()
      loss = -torch.gather(input=overall_likelihood,dim=1,index=stimulus.unsqueeze(1)).squeeze(1).log().sum()
 #     assert loss.abs() < 1e10, loss
  else:
    assert False
