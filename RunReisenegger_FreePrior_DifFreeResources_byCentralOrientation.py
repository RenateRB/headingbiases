...
def model(grid):
  ...
  for iteration in range(10000000):
   ...
   grid_cpu = grid.cpu()
   if iteration % 1000 == 0:
     figure, axis = plt.subplots(N_CO, 8, figsize=(20,20))
   ...
   #volumeBySubjectRandomAdjustment = torch.normal(MakeZeros(2,N_SUBJECTS,2*FOURIER_BASIS_SIZE), MakeZeros(2,N_SUBJECTS,2*FOURIER_BASIS_SIZE)+1) * (init_parameters["SIGMA_volumeBySubject"]+.1)
   ...
   logQPrior = -(priorByCORandomAdjustment / (init_parameters["SIGMA_priorByCO"]+.1) ).pow(2)/2 - (init_parameters["SIGMA_priorByCO"]+.1).log()
   if torch.isnan(logQPrior).any():
      print(f"Iteration: {iteration}")
      #print(f"priorByCORandomAdjustment: {priorByCORandomAdjustment}, init_parameters: {init_parameters['SIGMA_priorByCO']}")
      if torch.isnan(priorByCORandomAdjustment).any():
         print(f"Nans in priorByCORandomAdjustment.")
         print(f"priorByCORandomAdjustment: {priorByCORandomAdjustment}")
      elif torch.isnan(init_parameters["SIGMA_priorByCO"]).any():
         print(f"Nans in init_parameters.")
         print(f"init_parameters: {init_parameters['SIGMA_priorByCO']}")
      else:
         temp = logQPrior[torch.isnan(logQPrior)]
         count_temp = temp.size()
         print(f"{count_temp} Nans in logQPrior")
         #print(f"logQPrior: {logQPrior}")
   ...
   for CONDITION in range(2):
    for CO in range(N_CO):
     ...
     prior_overall = torch.nn.functional.softmax(parameters["prior"] + priorByCO[CO], dim=0)
    ...
    #  sigma_tmp = init_parameters["sigma_logit"][CONDITION]
    #  print(f"sigma_logit: {sigma_tmp}")
     ...
     if iteration % 1000 == 0:
       ...
       axis[CO][6].scatter(grid_centered.cpu(), y_smoothed.cpu())
#       axis[CO][6].scatter(grid_centered[MASK].cpu(), y_smoothed[MASK].cpu())
       axis[CO][7].scatter(x_here_centered.cpu(), bias.cpu(), s=0.1)
   ...
   if iteration % 1000 == 0:
     ...
     for CONDITION in range(2):
      ...
      volumeLogits = init_parameters["volume"][CONDITION].unsqueeze(0)
      ...
      regularizer_total[CONDITION] = regularizer1 + regularizer2
...
# Part: Initialize the model
  ...
  init_parameters["priorByCO"] = MakeZeros(N_CO,2*FOURIER_BASIS_SIZE)
