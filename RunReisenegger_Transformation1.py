...
#  print(f"sigma2: {sigma2}")
 ...
 #  print(f"sigma2: {sigma2}")
  ...
  error = (SQUARED_STIMULUS_SIMILARITY(grid.unsqueeze(0) - responses.unsqueeze(1)))
...
#  uniform_part = torch.sigmoid(parameters["mixture_logit"][centralOrientation])
 ...
 # motor_likelihoods = (1-uniform_part) * motor_likelihoods + (uniform_part / (2*math.pi) + 0*motor_likelihoods)
  ...
  overall_likelihood = torch.matmul(motor_likelihoods, torch.matmul(transfer_likelihoods, torch.matmul(mapping_likelihoods, likelihoods)))
#  print(mapping_likelihoods)
