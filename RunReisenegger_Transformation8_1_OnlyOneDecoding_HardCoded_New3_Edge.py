...
#  print(mapping_likelihoods)
  ...
  transform_FI = torch.softmax((0+init_parameters["f_t"][0])*SQUARED_STIMULUS_SIMILARITY(grid-180), dim=0).detach() #init_parameters["f_t"]
...
#   regularizer4 = prior_logits_effect_weight * (priorByCOFourierTimesWeightInclMean).pow(2).sum() - 0.5 * (N_CO*2*FOURIER_BASIS_SIZE) * log_prior_logits_effect_weight
   ...
   U = 1 + init_parameters["volume"][0,0,0]
   volume = SENSORY_SPACE_VOLUME * torch.nn.functional.softmax(U*SQUARED_STIMULUS_SIMILARITY(grid*2), dim=0) #.detach()
...
#       axis[CO,1].plot(grid_centered[0:179].cpu(), priorExpected[0:179].detach().cpu())
 #      axis[CO,1].plot(grid_centered[180:].cpu(), priorExpected[180:].detach().cpu())
       ...
       axis[3,6].scatter(grid_centered[MASK].cpu(), (EGO_bayesianEstimate_model-grid)[MASK].detach().cpu(), color=COLORS[CO][CONDITION])
...
# Part: Initialize the model
  ...
  init_parameters["sigma_logit"] = -5 + MakeZeros(N_CO, 2)
