...
#  print(f"sigma2: {sigma2}")
 ...
 if True:
  ...
  if condition_ == 1:
   ...
   #  print(mapping_likelihoods)
     ...
     sigma2_t2 = .0001 + 2*torch.sigmoid(init_parameters["sigma2_t2"]) #maybe change 2 for 4?
     F_t = torch.cat([MakeZeros(1), torch.cumsum(torch.softmax(init_parameters["volume"][0,0], dim=0), dim=0)], dim=0)
 ...
 # assert motor_likelihoods.log().abs().max() < 1e10, motor_variance
...
#       axis[CO,4].scatter(grid_centered.cpu(), (attraction).detach().cpu())
   ...
   if iteration % 1000 == 0:
     ...
     for CONDITION in range(2):
      ...
      for CO in range(N_CO):
       volume = SENSORY_SPACE_VOLUME * torch.nn.functional.softmax(parameters["volume"][0,0], dim=0)
       prior = torch.nn.functional.softmax(parameters["prior"] + init_parameters["priorByCO"][0,0], dim=0)
  ...
  #  print(init_parameters)
   ...
   #     loss_, bayesianEstimate, bayesianEstimate_sd, attraction, encodingBias = computeBias(xValues, init_parameters["sigma_logit"][0], prior, volume, n_samples=1000, grid=grid, responses_=observations_y, computePredictions=(iteration % 500 == 0), parameters=parameters, condition_=condition_, folds=testFolds, lossReduce='sum')
