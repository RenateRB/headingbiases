...
def softplus(x):
   return (torch.exp(x)+1).log() / math.log(2)
...
def model(grid):
  ...
  responseRangeMask = torch.where((grid-180).abs() < 110, 0*grid, 0*grid-5)
...
#    init_parameters["priorByCO"][0,CONDITION].data = 
    ...
    for CO in range(N_CO):
     ...
     # at CO = 0, separator = 0
     # at CO = -90, separator = 3*GRID//4
     # at CO = +90, separator = GRID//4
     separator = [int(-COs[CO]/MAX_GRID*GRID), 0, int((1-COs[CO]/MAX_GRID)*GRID)][CO]

     shiftedPrior = init_parameters["priorByCO"][0,CONDITION] * [1, softplus(init_parameters["prior_temperature"][0])][CONDITION]
...
#     plt.show()
     ...
     for CONDITION in range(2):
      ...
      for CO in range(N_CO):
       ...
       shiftedPrior = init_parameters["priorByCO"][CO,CONDITION] * [1, softplus(init_parameters["prior_temperature"][0])][CONDITION]
       shiftedPrior = torch.cat([shiftedPrior[separator:], shiftedPrior[:separator]], dim=0)
...
#  init_parameters["SIGMA_priorByCO"] = MakeZeros(N_CO,2*FOURIER_BASIS_SIZE)
  ...
  init_parameters["prior_temperature"] = MakeZeros(1)
