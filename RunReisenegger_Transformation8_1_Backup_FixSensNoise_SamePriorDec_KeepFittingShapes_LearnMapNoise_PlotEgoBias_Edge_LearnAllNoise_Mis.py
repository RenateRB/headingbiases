...
#  print(mapping_likelihoods)
  ...
  sigma2_t2 = 0.0001 + 2*torch.sigmoid(init_parameters["sigma2_t2"]) #maybe change 2 for 4?
...
def model(grid):
  ...
  responseRangeMask = torch.where((grid-180).abs() < 110, 0*grid, 0*grid-2)
...
#   regularizer4 = prior_logits_effect_weight * (priorByCOFourierTimesWeightInclMean).pow(2).sum() - 0.5 * (N_CO*2*FOURIER_BASIS_SIZE) * log_prior_logits_effect_weight
   ...
   for CONDITION in [1]:
    for CO in range(N_CO):
     ...
     prior = torch.nn.functional.softmax(C*SQUARED_STIMULUS_SIMILARITY(grid-COs[CO]+D-180) + responseRangeMask) #.detach()
